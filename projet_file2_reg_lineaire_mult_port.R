...
# avggrades ~  higher + school + studytime + schoolsup + 
# Medu + Dalc + absences + sex + health + guardian + famsize + 
#   Fjob + romantic + internet
summary(res_Stepwise0)
res_backwart0=step(resCompl,direction="backward")
# avggrades ~ school + sex + famsize + Medu + Fjob + guardian + 
# studytime + schoolsup + paid + higher + internet + romantic + 
#   famrel + freetime + Dalc + health + absences
summary(res_backwart0)

vif(resCompl)

#validation du modele
plot(res_Stepwise0)#analyse de résidus
shapiro.test(res_Stepwise0$residuals)#pvalue >5% donc on garde H0, nos donnΓ©es semble gaussien
bartlett.test(res_Stepwise0$residuals~famsize)#pvalue> 5% on garde H0 ,les residues semble homoscedastique
bartlett.test(res_Stepwise0$residuals~ Dalc)#pvalue< 5% on rejete H0 ,les residues ne semble pa homoscedastique
